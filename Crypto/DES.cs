...
namespace Crypto
{
    public static class BitArrayExtension
    {
        ...
        public static BitArray RightShift(this BitArray bit, int index)
        {
            BitArray bitArray = new BitArray(bit);
            for (int i = 0; i < bit.Length; i++)
            {
                bitArray[i] = bit[(i + index) % bit.Length];
            }
            return bitArray;
        }
    ...
    public class DES
    {
        ...
        /// Блок шифрования
        /// </summary>
        private class BlockEncode
        {
            //Propertys
            public BitArray BitArray { get; set; }
            public int Size { get { return BitArray.Length; } }
            //Ctor
            public BlockEncode(byte[] bytes)
            {
                BitArray = new BitArray(bytes);
            }
            public BlockEncode(BitArray bits)
            {
        ...
        /// Блок расшифровки
        /// </summary>
        private class BlockDecode
        {
            //Propertys
            public BitArray BitArray { get; set; }
            public int Size { get { return BitArray.Length; } }
            //Ctor
            public BlockDecode(byte[] bytes)
            {
                BitArray = new BitArray(bytes);
            }
            public BlockDecode(BitArray bits)
            {
                BitArray = new BitArray(bits);
            }
            //Methods
            ...
            public void Round(BitArray key, int roundIndex)
            {
                ...
                BitArray L = new BitArray(SubArray(bytes, 0, 4));
                BitArray R = new BitArray(SubArray(bytes, 4, 4));
                BitArray FL = F(L, key, roundIndex);
                R = R.Xor(FL);
                result.AddRange(GetBinary(R));
                result.AddRange(GetBinary(L));
                BitArray = new BitArray(result.ToArray());
        ...
        /// <returns></returns>
        private BlockEncode[] GetBlocksEncode(byte[] src)
        {
            int size = src.Length / 8;

            BlockEncode[] BlockEncodes = new BlockEncode[size];
            for (int i = 0; i < BlockEncodes.Length; i++)
            {
                BlockEncodes[i] = new BlockEncode(SubArray(src, i * 8, 8));
            }
        ...
        {
            BlockEncode[] BlockEncodes = GetBlocksEncode(Encoding.Unicode.GetBytes(src));
            List<byte> result = new List<byte>();
            foreach (BlockEncode BlockEncode in BlockEncodes)
            {
                BlockEncode.IP();
                //Console.WriteLine($"Key   :" + string.Join("-", _binaryKey.GetBytes().Select(x => x.ToString("X"))));
                for (int i = 0; i < 16; i++)
                    BlockEncode.Round(_binaryKey, i);
                BlockEncode.INV_IP();
                result.AddRange(BlockEncode.GetBytes());
            }
        ...
        {
            BlockDecode[] BlockEncodes = GetBlocksDecode(Encoding.Unicode.GetBytes(src));
            List<byte> result = new List<byte>();
            foreach (BlockDecode BlockEncode in BlockEncodes)
            {
                BlockEncode.IP();
                //Console.WriteLine($"Key   :" + string.Join("-", _binaryKey.GetBytes().Select(x => x.ToString("X"))));
                for (int i = 0; i < 16; i++)
        ...
        /// <param name="src"></param>
        /// <returns></returns>
        private BlockDecode[] GetBlocksDecode(byte[] src)
        {
            int size = src.Length / 8;

            BlockDecode[] BlockEncodes = new BlockDecode[size];
            for (int i = 0; i < BlockEncodes.Length; i++)
            {
                BlockEncodes[i] = new BlockDecode(SubArray(src, i * 8, 8));
            }
